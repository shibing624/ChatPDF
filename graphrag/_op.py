...
async def extract_entities(
    ...
    logger.info(f"Extracting entities from {len(ordered_chunks)} chunks")
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        logger.debug(
            ...
            f"{already_entities} entities(duplicated), {already_relations} relations(duplicated)"
        )
...
async def _pack_single_community_describe(
    ...
    # If context is larger than the limit and have sub-communities:
    report_describe = ""
...
async def generate_community_report(
    ...
    for level in levels:
        ...
        this_level_communities_reports = await tqdm_asyncio.gather(
            *[
...
async def _build_local_query_context(
    ...
    logger.info(
        f"Using {len(node_datas)} entites, {len(use_communities)} communities,"
...
async def local_query(
    ...
    logger.debug(f"query: {query}, sys_prompt: {sys_prompt}, prompt len: {len(sys_prompt)}")
